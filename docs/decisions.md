...
### D27. Render free tier: one web service, migrate-on-boot, sslmode in the URL, keep-warm pinger

**Decision:** The deploy is a single Render free-tier web service running `npm run build` / `npm start` from the GitHub repo, plus a Render managed Postgres. The one Express process serves REST, WebSocket, and the static frontend (D25's one-origin decision made deployable). Migrations run at boot — `schema.sql` is fully idempotent (`IF NOT EXISTS` everywhere), so every boot applies it and a release phase would add a moving part for nothing. TLS to Postgres is requested libpq-style: `?sslmode=require` appended to `DATABASE_URL`; `createPool` interprets the parameter itself and passes an explicit `ssl: { rejectUnauthorized: false }` to `pg` rather than trusting `pg`'s own sslmode parsing, whose semantics have shifted across versions — encryption without CA verification is exactly what libpq's "require" means, and Render's certs aren't in Node's default CA bundle. Production env vars: `DATABASE_URL`, `JWT_SECRET` (boot warns loudly if the public dev fallback is active), nothing else — bots default on, and no `ANTHROPIC_API_KEY` means the rule-based explainer serves the demo at zero marginal cost (D26). An external cron (cron-job.org) pings `GET /api/health` every 10 minutes to hold off the free tier's idle spin-down.

**Why:** The evaluation criterion is "a recruiter clicks a link and trades within 30 seconds" (CLAUDE.md §1) — a free, always-on-enough, zero-config public URL beats a fancier paid setup that lapses when the trial does. Everything about the architecture already tolerates this host: restarts are survivable by design (D15 reconciliation + D25 client resync — free-tier evictions are just more restarts), SIGTERM flushes the write-behind queue before exit, and spin-down is a documented limitation rather than a hidden one. The keep-warm pinger is external because an in-process timer cannot keep an instance alive that the platform has already suspended — the request must arrive from outside. Free Postgres expiring after ~30 days is accepted and stated in the README: the demo's value is the mechanism, not the data, and boot reseeds bots into a fresh database idempotently (D21).
