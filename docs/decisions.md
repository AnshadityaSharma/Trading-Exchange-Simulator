...
### D25. Frontend: same-origin static files; a reconnect always resyncs REST state

**Decision:** The BB Terminal frontend (built in Claude Design against the frozen contract) ships as static files in `web/`, served by the same express app (`express.static` after the API routes) — one origin, one deploy, relative `/api` and `/ws` URLs with zero client configuration. The prototype's mock data layer (`mock-exchange.js`) was deleted at its two marked swap points (`mockFetch`→`fetch`, `MockWebSocket`→`WebSocket`); the design's Tweaks panel (`simIntensity`/`edgeCases`) drove only the mock stream and was removed rather than wired to a bot-control endpoint the contract doesn't have. One behavior was *added* beyond the prototype: on every reconnect (not first connect), the client refetches open orders, account, tape, and stats alongside the contract-mandated resubscribe.

**Why:** Same-origin serving is what makes "recruiter clicks a link and trades in 30 seconds" literal — no CORS, no config, one URL (CLAUDE.md §1). The contract is frozen: where the design assumed anything the contract doesn't provide (the sim tweaks), the frontend changed, never the contract. The reconnect-resync exists because **a reconnect can hide a server restart**: the backend cancels orphaned open orders at boot *in the database* (D15) — no `order_update` is ever emitted for them, so a client that only resubscribes (all the contract strictly requires) would show open orders that no longer exist and stale reserved cash forever. Channels heal themselves via snapshot; REST-derived state does not. `e2e/restart.mjs` proves the full loop in a real browser: kill the server mid-session, the retry loop rides out the outage, the fresh snapshot replaces every stale book level (exact match against a REST snapshot in the same seq space), reconciled orders vanish, and reserved cash returns.

### D26. AI explainer: a zero-cost rule-based default, the LLM behind an env var

**Decision:** The `/orders/:id/explain` endpoint now has two implementations behind the same `Explainer` interface: `RuleBasedExplainer` (a pure, offline template engine) and `AnthropicExplainer` (the LLM one from D24). Selection is by environment: `ANTHROPIC_API_KEY` set → Anthropic, otherwise the rule-based default. Both read the *same* injected `ExplainData` facts (order side/type/limit, per-level executions, avg fill = filledNotional ÷ filledQty, price improvement or slippage across levels, unfilled remainder, and the maker/taker role that distinguishes a rested-then-filled limit from an aggressive sweep) and use the *same* rupee formatting as `buildPrompt`. The template engine covers every shape an order can end in — full fill, partial fill still resting, market sweep across levels, market remainder canceled, rested-then-filled, and a canceled limit — and always returns a non-empty sentence, so `INTERNAL` is reserved for the one genuinely unproducible case: the order's facts have vanished. The old `UnavailableExplainer` (which returned `INTERNAL` when no key was set) is gone.

**Why:** The public demo is the product (CLAUDE.md §1: "a recruiter clicks a link and trades"). Gating explanations on a paid API key meant the default deploy either cost money per click or showed an error where the marquee AI feature should be. A rule-based engine built from the exact same integer facts gives a real, correct explanation for free, instantly, with no external dependency or rate limit — the demo works out of the box, and richer LLM prose is one env var away for anyone who wants it. Keeping both behind one interface (and one shared data source) means the endpoint, the route, the cache semantics, and the contract behavior are identical regardless of which is active; only the phrasing differs, and neither can invent facts because both are handed the same numbers. One honest limitation, stated in code and tests: the persisted order record does **not** distinguish a self-trade-prevention cancel from a manual cancel (the frontend infers self-trade from its own client-side context, D25), so the explainer describes a canceled order's economics without asserting the cause — claiming "self-trade" from facts that don't prove it would be the kind of confident-but-wrong output the whole no-floats/exact-facts discipline exists to prevent.

---

## Phase 5 — Deployment

### D27. Railway web service + Neon Postgres: one process, migrate-on-boot through the pooler, DB-touching health check

**Decision:** The deploy is a single Railway web service (Singapore region) running `npm run build` / `npm start` from the GitHub repo, with Neon serverless Postgres (ap-southeast-1, same region) as the database. This supersedes an earlier Render+managed-Postgres plan that was never executed; the code changes made for it (sslmode handling, `PORT`/`DATABASE_URL` from env, engines pin, JWT warning) carry over unchanged. The one Express process serves REST, WebSocket, and the static frontend (D25's one-origin decision made deployable). Specifics:

- **`DATABASE_URL` is Neon's *pooled* connection string** (PgBouncer, `sslmode=require`). `createPool` interprets `sslmode` itself and passes an explicit `ssl: { rejectUnauthorized: false }` to `pg` rather than trusting `pg`'s own sslmode parsing, whose semantics have shifted across versions — encryption without CA verification is exactly what libpq's "require" means.
- **No `DIRECT_DATABASE_URL`.** Neon recommends the direct (non-pooled) URL for migration *tools* that need session state — advisory locks, named prepared statements, LISTEN. Our migration is one `pool.query()` of fully idempotent DDL (`IF NOT EXISTS` everywhere) sent as a single simple-protocol query, which PgBouncer transaction pooling executes on one server connection like any other query; verified by running the migration through the pooled URL before first deploy. A second env var would be configuration for a need we don't have.
- **Migrations run at boot**, not in a release step — idempotent schema means every boot applies it and a release phase would add a moving part for nothing.
- **`GET /api/health?deep=1` runs a `SELECT 1` and answers `{ "status": "ok", "db": "ok" }`.** Neon's free-tier compute suspends after ~5 idle minutes; a health endpoint that never touches the DB would keep the web process warm while the database slept, and the first real user query would pay the resume. The external keep-warm pinger (cron-job.org, every 10 minutes) hits the deep variant and therefore wakes both tiers with one request. Plain `/api/health` stays DB-free on purpose: it is the no-op control run in the HTTP benchmark methodology (D19), so it must measure the bare express layer, and the distinguishable `db` field means a pinger's success log actually proves database reachability rather than merely process liveness. The `deep` parameter is a server-internal, pinger-facing knob — the frozen contract's health entry (plain liveness) is unchanged for clients. Honest limit, stated in the README: a 10-minute interval doesn't beat a 5-minute suspend timer — it bounds staleness rather than eliminating resumes.
- **Production env vars: `DATABASE_URL`, `JWT_SECRET`** (boot warns loudly if the public dev fallback is active), nothing else — bots default on, and no `ANTHROPIC_API_KEY` means the rule-based explainer serves the demo at zero marginal cost (D26).

**Why:** The evaluation criterion is "a recruiter clicks a link and trades within 30 seconds" (CLAUDE.md §1) — a zero-config public URL on free/trial infrastructure beats a fancier paid setup. Railway's trial model (service runs until a one-time credit exhausts) avoids Render-style 15-minute web spin-downs, moving the cold-start problem to the database tier, which the `SELECT 1` health check addresses. Everything about the architecture already tolerates these hosts: restarts are survivable by design (D15 reconciliation + D25 client resync — platform evictions are just more restarts), SIGTERM flushes the write-behind queue before exit, and every remaining wart (Neon resume latency, credit exhaustion, crash-loss window) is a documented limitation rather than a hidden one. The pinger is external because an in-process timer cannot generate the outside traffic that keeps managed infrastructure awake.

### D28. Bot history retention: prune bot-vs-bot rows older than 2 days, hourly, in-process

**Decision:** A retention job (`src/db/retention.ts`) deletes bot-vs-bot **orders, fills, and trades** older than 2 days, running once at boot (fire-and-forget) and then hourly on an in-process timer. Rules, in order of precedence:

- **Rows touching a real human are never deleted.** A trade is prunable only if no fill on either side belongs to a non-bot user; fills are deleted only when bot-owned; orders only when bot-owned — and a bot order that ever traded against a human keeps its fills and is therefore kept itself, so both sides of every human trade stay explainable forever. Human history is tiny (22 fills at the time of writing) and is the part with demo value.
- **Accounting state (`balances`, `positions`) is never touched.** The prune removes journal rows, not ledger rows — money/inventory conservation (D16) is unaffected because conservation is a property of current balances, which bots carry forward across restarts regardless of how much of their old paper trail exists.
- **Each symbol's latest trade survives** even when ancient, because boot restores `lastPrice` from it (D22-era `loadTradeHistory`).
- **Only terminal orders** (`filled`, `canceled`) are candidates; open orders belong to the live book.

**Why:** The deploy's Neon free tier allows ~0.5 GB; measured three days after deploy, the database was 466 MB and growing ~145 MB/day, of which effectively 100% was bot flow — 825,386 of 825,388 orders and 550,204 of 550,226 fills belonged to the two bot accounts (the market maker's constant requoting dominates). Unbounded 24/7 bot history would exhaust the allowance in under four days. Two days is the retention floor with margin: boot replays only the last 24h of trades into the in-memory tape/stats, and the API serves the tape from memory, so nothing user-visible reads bot history older than that — steady state lands around 290 MB. Hourly cadence keeps each delete batch to roughly an hour of accrual (cheap on 0.25 CU), and the job is in-process (no cron infrastructure, CLAUDE.md §2) with failures logged and retried next tick — retention is maintenance, never a reason to take the exchange down. Rejected alternatives: partitioning by day (operational machinery the table sizes don't justify), TTL via `pg_cron` (not on Neon free tier), and not persisting bot flow at all (bots are ordinary users by design, D16 — special-casing their persistence would fork the write path the whole architecture avoids).

### D29. Migrated to a fresh Neon project after the deep health pinger burned the CU-hour budget

**Decision:** Moved the database from Neon project `exchange-pg` (exhausted its free monthly CU-hours) to a fresh project `exchange-pg-2`. Root cause: the keep-warm pinger hitting `GET /api/health?deep=1` every 10 minutes ran `SELECT 1` and so kept Neon's compute from ever scaling to zero, billing compute around the clock. Going forward the cron pings **shallow `/api/health` only** (DB-free, D27) — Neon scales to zero when idle and visitors occasionally pay the ~1–2s resume, which is the honest free-tier tradeoff we already document. The copy was **lean**, done by a one-off script reading OLD in a single `REPEATABLE READ` snapshot (it is the live prod DB, still taking bot flow): full ledger (`users`, `balances`, `positions` — all 4 accounts, byte-exact) and all human order/fill history copied verbatim; `trades` trimmed to the 48h the retention window keeps; ~530k bot orders / ~350k bot fills dropped (D28 deletes them anyway). `users_id_seq` reset via `setval(max(id))` so the next signup can't collide. Result: 35 MB on NEW vs ~460 MB on OLD.

**Why:** A fresh project resets the CU-hour clock immediately, and the pinger change removes the thing that spent it — the two together are the fix, not either alone. Verification compared NEW against the same OLD snapshot and matched exactly: balances/positions row-for-row, total system cash `11,200,000,000` paise, and per-symbol inventory/cost-basis/realized-pnl identical. One caveat recorded honestly: per-symbol inventory sums to ~11,980, not the seeded 12,000 — a few bot lots are in flight in the in-memory engine and not yet flushed at snapshot time (write-behind, D-async), so OLD's *committed* state is slightly behind the live truth. That is a faithfully-copied pre-existing property of the source, not a migration error; the migration's contract is an exact copy of committed state, and OLD and NEW agree to the byte. Rejected: a full `pg_dump` copy (would carry the bot noise straight back toward the cap on day one, and `pg_dump` isn't installed on the dev box anyway); quiescing prod for a clean snapshot first (chicken-and-egg with the cutover, and the snapshot isolation makes it unnecessary).

### D30. Demand-gated bots + retention, and a secret-gated deep health check, so Neon scales to zero when unwatched

**Decision:** Even after D29, `exchange-pg-2`'s compute ran flat 24/7 (~0.25 CU). Root cause this time was the bot fleet itself: continuous bot flow → write-behind flushing every 50ms → Neon never sees a 5-minute idle window. Fix is to make the market **demand-driven**. A `Presence` (`src/server/presence.ts`) holds the timestamp of the last genuine client interaction; the HTTP middleware touches it on every request **except `/api/health`**, and the WS layer touches it on connect and every client frame. Bots gate each tick on `presence.isActive()` (5-minute window) — idle means no submit and no cancel, so nothing enqueues and write-behind's flush is a no-op on empty buffers (it already returns before `pool.connect()`); the retention job gates the same way and skips its hourly prune while idle. With no visitors, all writers fall silent, Neon suspends, and the first visitor pays one ~1–2s cold start (accepted). Bots start **idle** at boot, so a freshly deployed instance with no traffic never writes.

Alongside, the DB-waking deep health check is hardened: `GET /api/health?deep=1` now runs `SELECT 1` **only** when `&key=` matches `HEALTH_DEEP_KEY` (env); an absent or wrong key **degrades to the shallow, DB-free `{status:'ok'}`** rather than erroring. Shallow `/api/health` stays fully public and DB-free.

**Why:** An audit of every periodic timer (bot maker/noise, retention, write-behind flush, WS idle-sweeper) confirmed only the first three can issue SQL, and all three are now presence-gated; write-behind self-gates on empty buffers, and the WS sweeper is in-memory only. That left exactly one way to wake an idle database — the deep health endpoint — and it was unauthenticated, i.e. the same class of stray traffic that caused the 24/7 bill in D29. Degrading (not erroring) on a bad key is deliberate: a misconfigured or stale pinger should become harmless, not start returning 4xx/5xx that trips uptime alarms. The secret lives in an env var (like `JWT_SECRET`) so it's rotatable and never in the repo. Net: with zero visitors, **nothing** touches Postgres, and the invariant can't be silently broken by external config. Cost of the presence indirection is one timestamp compare per tick — negligible. The one honest seam: a real viewer passively watching still counts, because the frontend pings the WS every ≤25s and polls stats every 5s — which is correct (someone *is* watching), and it's the bare health pinger, with no page and no socket, that must not count.

**Ops note:** set `HEALTH_DEEP_KEY` to a random secret in Railway. Keep the cron-job.org pinger on plain `/api/health` (shallow, DB-free) — the deep+key URL is for on-demand DB-reachability checks only, not the routine keep-warm ping (there is no keep-warm anymore by design).
