...
- **`GET /api/health` runs a `SELECT 1`.** Neon's free-tier compute suspends after ~5 idle minutes; a health endpoint that never touches the DB would keep the web process warm while the database slept, and the first real user query would pay the resume. The external keep-warm pinger (cron-job.org, every 10 minutes) therefore wakes both tiers with one request. Honest limit, stated in the README: a 10-minute interval doesn't beat a 5-minute suspend timer — it bounds staleness rather than eliminating resumes.
